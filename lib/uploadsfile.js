...
async function uploadFile(filename) {
  ...
  try {
    ...
    // Klik tombol Upload
    await page.waitForSelector('#upLoad', { timeout: 60000 }); // Pastikan tombol upload ada
    const uploadButton = await page.$('#upLoad');
    if (!uploadButton) {
      console.error('[SFILE] ❌ Tombol upload tidak ditemukan');
      await browser.close();
      return null;
    }
    ...
    await page.waitForSelector('input[type="text"]', { timeout: 120000 }); // Menunggu lebih lama
