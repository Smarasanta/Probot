...
const autoPromosi = async (A17, global) => {
    try {
        ...
        for (let i = 0; i < anu.length; i++) {
            ...
            try {
				if (promoText) {
					await sendWithContext(groupId, promoText, 'Promo Terbaru dari ${global.BotName}');
					await new Promise(res => setTimeout(res, randomDelay(2000, 3000)));
				}
				if (autoscriptText) {
					await sendWithContext(groupId, autoscriptText, 'Autoscript Tunneling by ${global.BotName}');
					await new Promise(res => setTimeout(res, randomDelay(2000, 3000)));
				}
				if (recodeText) {
					await sendWithContext(groupId, recodeText, 'Jasa Recode ${global.BotName}');
					await new Promise(res => setTimeout(res, randomDelay(2000, 3000)));
				}
				if (vpsText) {
					await sendWithContext(groupId, vpsText, 'VPS ${global.BotName}');
					await new Promise(res => setTimeout(res, randomDelay(2000, 3000)));
				}
