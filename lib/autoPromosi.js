...
const { generateGroupBanner } = require('./bannerUtil');
...
const autoPromosi = async (A17, global) => {
    try {
        ...
        for (let i = 0; i < anu.length; i++) {
            ...
            try {
				if (promoText) {
					await sendWithContext(groupId, promoText, 'Promo Terbaru dari SPS');
					await new Promise(res => setTimeout(res, randomDelay(2000, 3000)));
				}
				if (autoscriptText) {
					await sendWithContext(groupId, autoscriptText, 'Autoscript Tunneling by SPS');
					await new Promise(res => setTimeout(res, randomDelay(2000, 3000)));
				}
				if (recodeText) {
					await sendWithContext(groupId, recodeText, 'Jasa Recode SPS');
					await new Promise(res => setTimeout(res, randomDelay(2000, 3000)));
				}
				if (vpsText) {
					await sendWithContext(groupId, vpsText, 'VPS SPS');
					await new Promise(res => setTimeout(res, randomDelay(2000, 3000)));
				}
